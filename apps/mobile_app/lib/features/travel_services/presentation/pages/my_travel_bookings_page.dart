...
final travelBookingsProvider =
    FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
      final response = await ref
          .watch(apiClientProvider)
          .dio
          .get(ApiEndpoints.travelServiceBookings);
...
class MyTravelBookingsPage extends ConsumerWidget {
      ...
      body: RefreshIndicator(
        ...
        child: bookings.when(
          ...
          error: (_, _) => ListView(
            children: const [
              SizedBox(height: 100),
              Center(child: Text('Could not load bookings.')),
            ],
          ),
          data: (items) => items.isEmpty
              ? ListView(
                  children: const [
                    SizedBox(height: 100),
                    Center(
                      child: Text('No hotel, flight, or bus bookings yet.'),
                    ),
                  ],
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final rawOffering = booking['offering_details'];
                    final offering = rawOffering is Map
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${offering['service_type']} • ${booking['reference']}',
                              style: const TextStyle(
                            ...
                            if (booking['start_date'] != null)
                              Text(
                                '${DateFormat.yMMMd().format(DateTime.parse(booking['start_date'] as String))} – ${DateFormat.yMMMd().format(DateTime.parse(booking['end_date'] as String))}',
                              ),
  ...
  Future<void> _cancel(WidgetRef ref, int id) async {
    await ref
        ...
        .dio
        .post('${ApiEndpoints.travelServiceBookings}$id/cancel/');
    ref.invalidate(travelBookingsProvider);
  }

  Future<void> _pay(BuildContext context, WidgetRef ref, int id) async {
    final paid = await startTargetPayment(
      context,
      ref,
      target: {'service_booking': id},
    );
